...
public class IPSeeker {
	...
	public IPSeeker (String url) {
		...
		try {
			ipFile = new RandomAccessFile(url, "r");
		}
		catch (FileNotFoundException e) {
			// 如果找不到这个文件，再尝试再当前目录下搜索，这次全部改用小写文件名
			//     因为有些系统可能区分大小写导致找不到ip地址信息文件
			String filename = new File("QQWry.dat").getName().toLowerCase();
	 ...
	 * @return 单一实例
	 * 
	 * public static IPSeeker getInstance () { return instance; }
	 **********************************************************************************************/

	/**
	 * 根据文件头的两个偏移相减并且除以7得到数据文件中数据总数
	 * 
	 * @return 记录总数
	 */
	public int getRecords () {
		int records;
		long begin = readLong4(0);
		long end = readLong4(4);
		long temp = end - begin;
		records = new Long(temp).intValue() / 7;
		return records + 1;
	}

	/**
	 * 最后一个记录的地区信息字段是更新日期信息
	 * 
	 * 也可以直接调用getArea("255.255.255.255")来获得因为此IP就是记录中的最后一个
	 * 
	 * @return 更新日期
	 */
	public String getUpdateDate () {
		long offset = readLong3(ipEnd + 4);
		IPLocation loc = getIPLocation(new Long(offset).intValue());
		return loc.area;
	}

	/**
	 * 最后一个记录的国家信息字段是数据库的版本信息
	 * 
	 * 也可以直接调用getCountry("255.255.255.255")来获得因为此IP就是记录中的最后一个
	 * 
	 * @return 数据库版本
	 */
